...
const StudentDetails = ({ student, goBack }) => {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/erp/students/${student._id}`, {
        method: 'PUT',
        headers: {
        ...
        {activeTab === 'applications' && (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '30px' }}>
              ...
              <button type="button" onClick={handleSaveProfile} className="btn-save" style={{ padding: '12px 30px', fontSize: '1.05rem' }}>
                <Save size={18} /> Save Complete Profile
              </button>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (documentUploadRef.current) {
                        setIsUploading(true);
                        setUploadProgress(5);
                        await documentUploadRef.current.executeProcessing((p) => setUploadProgress(p), formData);
                        setTimeout(() => {
